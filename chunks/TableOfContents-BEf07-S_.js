import { c as createAstro, a as createComponent, m as maybeRenderHead, d as addAttribute, b as renderTemplate, e as renderSlot } from './astro-server-D50-taHx.js';
import 'piccolore';
import 'clsx';
/* empty css                             */
import { e as escape_html, a as ensure_array_like, b as attr_class, c as attr } from './_at-astro-renderers-DylNdryL.js';
...
function ExpandableReports($$renderer, $$props) {
	...
	for (let index = 0, $$length = each_array.length; index < $$length; index++) {
		...
		if (report.tags && report.tags.length > 0) {
			$$renderer.push(`<!--[0--><div class="report-item__tags svelte-1warup"><!--[-->`);
		...
		if (expandedIndices.includes(index)) {
			$$renderer.push(`<!--[0--><div class="report-item__content svelte-1warup"><p class="svelte-1warup">${escape_html(report.description)}</p></div>`);
		} else {
			$$renderer.push('<!--[-1-->');
		}
...
function TableOfContents($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (title) {
			$$renderer.push(`<!--[0--><h3 class="toc__title svelte-1pbsi0n">${escape_html(title)}</h3>`);
		} else {
			$$renderer.push('<!--[-1-->');
		}
		...
		for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
			...
			if (section.icon) {
				$$renderer.push(`<!--[0--><span class="toc__icon svelte-1pbsi0n" aria-hidden="true">${escape_html(section.icon)}</span>`);
			} else {
				$$renderer.push('<!--[-1-->');
			}
