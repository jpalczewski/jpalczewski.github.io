import { c as createAstro, a as createComponent, r as renderComponent, b as renderTemplate, e as renderSlot, m as maybeRenderHead, d as addAttribute } from './astro-server-D50-taHx.js';
import 'piccolore';
import { $ as $$Layout } from './Layout-B5CQ06TI.js';
import { c as getSymmetricAlternate } from './content-E3HRhPnq.js';
...
const $$AboutLayout = createComponent(($$result, $$props, $$slots) => {
  ...
  const secondaryNavConfig = createAboutNavConfig(locale, { showSummary, additionalLinks });
  const alternate = getSymmetricAlternate(Astro2.url.origin, Astro2.url.pathname, locale);
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": title, "description": description, "pageType": "about", "secondaryNavConfig": secondaryNavConfig, "alternates": [alternate] }, { "default": ($$result2) => renderTemplate`  ${maybeRenderHead()}<div class="plain-content"> ${renderSlot($$result2, $$slots["default"])} </div> `, "head": ($$result2) => renderTemplate`${renderSlot($$result2, $$slots["head"])}` })}`;
