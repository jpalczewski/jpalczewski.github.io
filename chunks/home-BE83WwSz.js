...
import { p as parsePostId, r as renderEntry, e as generateExcerpt, a as getPostUrl, d as getCurrentLang } from './content-E3HRhPnq.js';
...
const PREVIEW_ASSET_PREFIX = "/__preview_asset__/";

const rawImageModules = /* #__PURE__ */ Object.assign({"/src/assets/images/pingu-teraz-juz-nie-chce.png": __vite_glob_0_0,"/src/assets/images/resident-advisor.png": __vite_glob_0_1


});
const previewImageMap = Object.fromEntries(
  Object.entries(rawImageModules).map(([path, mod]) => [
    path.slice("/src/assets/".length),
    mod.default.src
  ])
);
const PREVIEW_ASSET_RE = new RegExp(
  PREVIEW_ASSET_PREFIX.replace(/\//g, "\\/") + `assets\\/images\\/([^"'\\s>]+)`,
  "g"
);
function resolvePreviewImages(html) {
  return html.replace(PREVIEW_ASSET_RE, (_, filename) => previewImageMap[`images/${filename}`] ?? "");
}
async function mapPostsToHomePosts(posts, locale) {
  return Promise.all(posts.map(async (post) => {
    ...
    return {
      ...
      bodyHtml: resolvePreviewImages((await renderEntry(post)).remarkPluginFrontmatter.previewHtml ?? ""),
      tags: post.data.tags,
