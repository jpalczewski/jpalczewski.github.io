...
import 'piccolore';
import { $ as $$Layout } from './Layout-CBBsXRO0.js';
import { g as getCollection, h as getPostsForLocale, A as getUniqueYears, B as getUniqueYearMonths } from './content-C6-_BOYf.js';
...
const $$ThoughtsLayout = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { title, description, locale, showMonthly = true, structuredData, ogType, article, alternates } = Astro2.props;
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": title, "description": description, "pageType": "thoughts", "secondaryNavConfig": secondaryNavConfig, "structuredData": structuredData, "ogType": ogType, "article": article, "alternates": alternates }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="plain-content"> ${renderSlot($$result2, $$slots["default"])} </div> ` })}`;
